# -*- coding: utf-8 -*-
"""
Created on Wed Nov  2 00:36:32 2022
...
def sim_EV(df_inp_veh, df_inp_cycle, grdnt, MotorEffData):
  ...
  Batt_volt_nom = np.full(time.size, Batt_volt_nom)
  ...
  Wheel_Tq = ((mass * acc) + (a) + (b * speed) + (c * speed * speed) +
              (mass * 9.81 * np.sin(np.arctan(grad)))) * wheel_radius
  Wheel_Tq[speed == 0] = 0
  ...
  Emac_Tq[Wheel_Tq <
          0] = Wheel_Tq[Wheel_Tq < 0] / GR / eff_GB * brk_dstr * regen_fact
  ...
  Emac_eff_vals[Emac_eff_vals < np.nanmin(MotorEffData.values)] = np.mean(
    Emac_eff_vals)
  # Emac_eff_vals = np.full(len(time) , eff_Mot)

  Batt_Pwr_trxn = np.zeros(len(time))
  Batt_Pwr_trxn[
    Wheel_Tq >= 0] = Emac_Pwr[Wheel_Tq >= 0] / Emac_eff_vals[Wheel_Tq >= 0]
  Batt_Pwr_trxn[
    Wheel_Tq < 0] = Emac_Pwr[Wheel_Tq < 0] * Emac_eff_vals[Wheel_Tq < 0]
  ...
  df_op['Batt_i'] = Batt_i
  df_op['Batt_volt_nom'] = Batt_volt_nom
...
def calc(df_inp):
  ...
  #### Energy ####
  y_Ene_data = np.array([
    HVB_power_kW, sensor_HVB_Pwr_kW, Motor_power_trxn_kW, Shaft_power_kW,
    Shaft_powerDes_kW
  ])
  ...
  df_ene = Ene_masks(time, y_Ene_data, y_Ene_data_names, mask1_arr,
                     mask1_arr_names, Ene_output_name, Ene_Units)
  ...
  df_dist = Dist_masks(time, y_Distance_data, y_Distance_data_names, mask1_arr,
                       mask1_arr_names, Dist_output_name, Dist_Units)
  ...
  df_eneNorm.rename(columns=dict(
    zip(df_eneNorm_names_old, df_eneNorm_names_new)),
                    inplace=True)

  #### Stats ####
  stats_veh_vel_kmph = get_stats(veh_vel_kmph,
                                 prefix='veh_vel_kmph',
                                 perc=99.7)
  stats_veh_acc_mps2 = get_stats(veh_acc_mps2,
                                 prefix='veh_acc_mps2',
                                 perc=99.7)
  stats_HVB_i_A = get_stats(HVB_i_A, prefix='HVB_i_A', perc=99.7)
  stats_Shaft_power_kW = get_stats(Shaft_power_kW,
                                   prefix='Shaft_power_kW',
                                   perc=99.7)
  stats_Shaft_powerDes_kW = get_stats(Shaft_powerDes_kW,
                                      prefix='Shaft_powerDes_kW',
                                      perc=99.7)
  stats_Motor_power_kW = get_stats(Motor_power_kW,
                                   prefix='Motor_power_kW',
                                   perc=99.7)
  stats_HVB_power_kW = get_stats(HVB_power_kW,
                                 prefix='HVB_power_kW',
                                 perc=99.7)
  stats_APP_r = get_stats(APP_r, prefix='APP_r', perc=99.7)
  stats_Motor_tqDes_Nm = get_stats(Motor_tqDes_Nm,
                                   prefix='Motor_tqDes_Nm',
                                   perc=99.7)
  ...
  df_op_scalars = pd.concat([
    df_op_scalars, df_ene, df_dist, df_eneNorm, stats_veh_vel_kmph,
    stats_veh_acc_mps2, stats_Shaft_power_kW, stats_Shaft_powerDes_kW,
    stats_Motor_power_kW, stats_HVB_power_kW, stats_APP_r,
    stats_Motor_tqDes_Nm, stats_Motor_tq_Nm, stats_HVB_i_A
  ],
                            axis=1)

  df_op_scalars['PTEff_Net_%'] = round(
    df_op_scalars['ShaftDes_Energy_net_Wh'] /
    df_op_scalars['HVB_Energy_net_Wh'] * 100, 2)
  df_op_scalars['PTEff_boost_%'] = round(
    df_op_scalars['ShaftDes_Energy_boost_Wh'] /
    df_op_scalars['HVB_Energy_boost_Wh'] * 100, 2)
  df_op_scalars['PTEff_regen_%'] = round(
    df_op_scalars['HVB_Energy_regen_Wh'] /
    df_op_scalars['ShaftDes_Energy_regen_Wh'] * 100, 2)

  df_op_scalars['HVB_SoCinit'] = round(df_op['HVB_SoC'].iloc[0], 2)
  df_op_scalars['HVB_SoCdelta'] = round(
    df_op['HVB_SoC'].iloc[0] - df_op['HVB_SoC'].iloc[-1], 2)

  df_op_scalars['12V_Power_avg_W'] = round(
    np.mean(df_op['DCDC_power_kW'] * 1000), 2)
  df_op_scalars['HVB_Voltage_init'] = round(df_op['HVB_u'].iloc[0], 2)
  df_op_scalars['Cycle_duration_mins'] = round(
    (df_op['time'].iloc[-1] - df_op['time'].iloc[0]) / 60, 2)

  return [df_op, df_op_scalars]


def Sim_Fwd(df_inp, df_inp_cycle, grdnt, Motor_Data, MotorEffData):
  ...
  eff_Mot = float(df_inp['eff_Mot'])
  grad = grdnt
  Batt_volt_nom = float(df_inp['Batt_volt_nom'])
  acc_lim = float(df_inp['acc_lim'])
  time_lim = float(df_inp['time_lim'])
  time_rate = float(df_inp['time_rate'])
  Aux_pwr = float(df_inp['Aux_pwr'])
  regen_fact = float(df_inp['regen_fact'])
  brk_dstr = float(df_inp['brk_dstr'])

  time = df_inp_cycle['Time'].to_numpy()
  veh_speed_target = df_inp_cycle['Speed'].to_numpy()

  RPM_Data = Motor_Data[Motor_Data.columns[0]].values
  Tq_Data_1 = Motor_Data[Motor_Data.columns[2]].values
  Tq_Data_2 = Motor_Data[Motor_Data.columns[6]].values

  veh_speed = np.zeros(len(time))
  veh_speed[0] = veh_speed_target[0]
  ...
  interp1_MotorData_1 = interp1d(RPM_Data,
                                 Tq_Data_1,
                                 kind='linear',
                                 bounds_error=False,
                                 fill_value=0)
  ...
  for i in range(len(time) - 1):
    ...
    RPM_target[i] = veh_speed_target[
      i + 1] * 5 / 18 / wheel_radius * GR * 60 / 2 / 3.14
    RPM_current[i] = veh_speed[i] * 5 / 18 / wheel_radius * GR * 60 / 2 / 3.14

    acc_target[i] = (veh_speed_target[i + 1] -
                     veh_speed[i]) * 5 / 18 / (time[i + 1] - time[i])
    Tq_wheel_Prop[i] = acc_target[i] * mass * wheel_radius
    Wheel_Tq_RL[i] = ((a) + (b * veh_speed[i]) +
                      (c * veh_speed[i] * veh_speed[i]) +
                      (mass * 9.81 * np.sin(np.arctan(grad)))) * wheel_radius
    ...
    if acc_target[i] > 0:
      Wheel_Tq[i] = np.min(
        np.array([
          Wheel_Tq_RL[i] + Tq_wheel_Prop[i],
          Tq_mot_temp_available[i] * eff_GB * GR
        ]))
    else:
    ...
    veh_speed[i + 1] = veh_speed[i] + (acc_res[i] *
                                       (time[i + 1] - time[i]) * 18 / 5)

  Emac_Tq_desired = (Wheel_Tq_RL + Tq_wheel_Prop) / eff_GB / GR
  Emac_Tq_desired[
    Wheel_Tq < 0] = Emac_Tq_desired[Wheel_Tq < 0] * brk_dstr * regen_fact

  Emac_RPM = veh_speed * 5 / 18 / wheel_radius * GR * 60 / 2 / 3.14
  Emac_Tq_Nm = np.zeros(len(Wheel_Tq))
  Emac_Tq_Nm[Wheel_Tq >= 0] = Wheel_Tq[Wheel_Tq >= 0] / GR / eff_GB
  Emac_Tq_Nm[Wheel_Tq <
             0] = Wheel_Tq[Wheel_Tq < 0] / GR / eff_GB * brk_dstr * regen_fact
  ...
  Emac_eff_vals = eff_vals(MotorEffData, Emac_Tq_Nm, Emac_RPM)
  Emac_eff_vals[Emac_eff_vals < np.nanmin(MotorEffData.values)] = np.mean(
    Emac_eff_vals)
  # Emac_eff_vals = np.full(len(time) , eff_Mot)
  ...
  Batt_Pwr_trxn = np.zeros(len(time))
  Batt_Pwr_trxn[
    Wheel_Tq >= 0] = Emac_Pwr_kW[Wheel_Tq >= 0] / Emac_eff_vals[Wheel_Tq >= 0]
  Batt_Pwr_trxn[
    Wheel_Tq < 0] = Emac_Pwr_kW[Wheel_Tq < 0] * Emac_eff_vals[Wheel_Tq < 0]
  Batt_Pwr = Batt_Pwr_trxn + (Aux_pwr / 1e3)
  Batt_i = Batt_Pwr / Batt_volt_nom * 1e3
  ...
  df_op['Emac_Tq_desired'] = Emac_Tq_desired

  return df_op


def Ene_masks(x_data, y_data, y_data_names, mask1, mask1_names, output_name,
              Units):
  op_list = []
  ...
  for i in range(len(y_data_names)):
    for j in range(len(mask1_names)):
      ...
      varname_temp = y_data_names[i] + '_' + output_name + '_' + mask1_names[
        j] + '_' + Units
      locals()[varname_temp] = integrate.simps(y_data[i] * mask1[j],
                                               x_data) / 3.6
      #return locals()[varname_temp]
      #print ('Calculated '+  varname_temp, end=" ", flush=True)
      df_op_ene[iter_no] = [locals()[varname_temp]]
      df_op_ene = df_op_ene.rename(
        columns={df_op_ene.columns[iter_no]: varname_temp})
      iter_no = iter_no + 1

  return df_op_ene


def Dist_masks(x_data, y_data, y_data_names, mask1, mask1_names, output_name,
               Units):
  op_list = []
  df_op_ene = pd.DataFrame([])
  iter_no = 0
...
def eff_vals(MotorEffData_1, Mot_trq_1, RPM_mot):
  ...
  interp2 = interpolate.RectBivariateSpline(MotorEffData_Tq,
                                            MotorEffData_RPM,
                                            MotorEffData_Eff,
                                            kx=1,
                                            ky=1,
...
path_cycle_WLTP = 'C:/Users/MIH1BAN/Documents/Drive Cycles/WLTP/WLTC_Class1.xlsx'
path_cycle_MIDC = 'C:/Users/MIH1BAN/Documents/Drive Cycles/MIDC_Part1.xlsx'
path_cycle_topspeed1 = 'C:/Users/MIH1BAN/Documents/Drive Cycles/0to55_26s.xlsx'
path_cycle_rest_grad1 = 'C:/Users/MIH1BAN/Documents/Drive Cycles/0to10_7s.xlsx'
path_cycle_overtake1 = 'C:/Users/MIH1BAN/Documents/Drive Cycles/overtake_30to50_v1.xlsx'  #Jeeto
path_cycle_overtake2 = 'C:/Users/MIH1BAN/Documents/Drive Cycles/overtake_20to30_v1.xlsx'  #Jeeto
path_cycle_overtake3 = 'C:/Users/MIH1BAN/Documents/Drive Cycles/overtake_30to40_v1.xlsx'  #Jeeto
path_cycle_contSpeeds = 'C:/Users/MIH1BAN/Documents/Drive Cycles/cont_speeds.xlsx'  #Jeeto

time_column_tag = 'Time'
speed_column_tag = 'Speed'

MotorEffData1 = pd.read_excel(
  'C:/Users/MIH1BAN/Documents/ePT/IRP_G26W_Eff.xlsx')
MotorEffData1.set_index('Index', inplace=True, drop=True)

MotorEffData2 = pd.read_excel(
  'C:/Users/MIH1BAN/Documents/MnM_eLCV/Inputs/EAU150_Eff_350V.xlsx')
MotorEffData2.set_index('Index', inplace=True, drop=True)

Motor_Data1 = pd.read_excel(
  'C:/Users/MIH1BAN/Documents/MnM_eLCV/Inputs/EAU100_curves_350V.xlsx')
Motor_Data2 = pd.read_excel(
  'C:/Users/MIH1BAN/Documents/MnM_eLCV/Inputs/EAU150_curves_350V.xlsx')
...
Motor_Data5 = pd.read_excel(
  'C:/Users/MIH1BAN/Documents/ePT/InputData/IRP_PerfCurves_2.xlsx',
  sheet_name='IRPG2')
Motor_Data6 = pd.read_excel(
  'C:/Users/MIH1BAN/Documents/ePT/InputData/TML_reqCurves_v1.xlsx')

# Inputs

df_inp_veh1 = pd.DataFrame([])
df_inp_veh1['mass'] = [1500]
df_inp_veh1['a'] = 214.62
df_inp_veh1['b'] = 0
...
df_inp_veh3 = df_inp_veh1.copy()
df_inp_veh3['mass'] = 1700
df_inp_veh3['a'] = 243.236

df_inp_veh2 = df_inp_veh1.copy()
df_inp_veh2['mass'] = 1150
df_inp_veh2['a'] = 164.542

######################

df_inp_cycle1 = pd.read_excel(path_cycle_WLTP)
df_inp_cycle2 = pd.read_excel(path_cycle_MIDC)
df_inp_cycle3 = pd.read_excel(path_cycle_topspeed1)
df_inp_cycle4 = pd.read_excel(path_cycle_rest_grad1)
df_inp_cycle5 = pd.read_excel(path_cycle_overtake2)
df_inp_cycle6 = pd.read_excel(path_cycle_overtake1)
df_inp_cycle7 = pd.read_excel(path_cycle_overtake3)
df_inp_cycle8 = pd.read_excel(path_cycle_contSpeeds)

####################

MotData_selected = Motor_Data5
MotData_selected2 = Motor_Data6

[df_op_11, df_op_scalars_11] = sim_EV(df_inp_veh1, df_inp_cycle1, 0,
                                      MotorEffData1)
[df_op_21, df_op_scalars_21] = sim_EV(df_inp_veh1, df_inp_cycle2, 0,
                                      MotorEffData1)
[df_op_31, df_op_scalars_31] = sim_EV(df_inp_veh1, df_inp_cycle3, 0,
                                      MotorEffData1)
[df_op_41, df_op_scalars_41] = sim_EV(df_inp_veh1, df_inp_cycle4, 0.2016,
                                      MotorEffData1)
[df_op_51, df_op_scalars_51] = sim_EV(df_inp_veh1, df_inp_cycle4, 0.1228,
                                      MotorEffData1)
[df_op_61, df_op_scalars_61] = sim_EV(df_inp_veh1, df_inp_cycle5, 0,
                                      MotorEffData1)
[df_op_71, df_op_scalars_71] = sim_EV(df_inp_veh1, df_inp_cycle6, 0,
                                      MotorEffData1)
[df_op_81, df_op_scalars_81] = sim_EV(df_inp_veh1, df_inp_cycle7, 0.1228,
                                      MotorEffData1)
[df_op_91, df_op_scalars_91] = sim_EV(df_inp_veh1, df_inp_cycle8, 0,
                                      MotorEffData1)

[df_op_12, df_op_scalars_12] = sim_EV(df_inp_veh2, df_inp_cycle1, 0,
                                      MotorEffData1)
[df_op_22, df_op_scalars_22] = sim_EV(df_inp_veh2, df_inp_cycle2, 0,
                                      MotorEffData1)
[df_op_32, df_op_scalars_32] = sim_EV(df_inp_veh2, df_inp_cycle3, 0,
                                      MotorEffData1)
[df_op_42, df_op_scalars_42] = sim_EV(df_inp_veh2, df_inp_cycle4, 0.2016,
                                      MotorEffData1)
[df_op_51, df_op_scalars_51] = sim_EV(df_inp_veh2, df_inp_cycle4, 0.1228,
                                      MotorEffData1)
[df_op_62, df_op_scalars_62] = sim_EV(df_inp_veh2, df_inp_cycle5, 0,
                                      MotorEffData1)
[df_op_72, df_op_scalars_72] = sim_EV(df_inp_veh2, df_inp_cycle6, 0,
                                      MotorEffData1)
[df_op_82, df_op_scalars_82] = sim_EV(df_inp_veh2, df_inp_cycle7, 0.1228,
                                      MotorEffData1)
[df_op_92, df_op_scalars_92] = sim_EV(df_inp_veh2, df_inp_cycle8, 0,
                                      MotorEffData1)

[df_op_13, df_op_scalars_13] = sim_EV(df_inp_veh3, df_inp_cycle1, 0,
                                      MotorEffData1)
[df_op_23, df_op_scalars_23] = sim_EV(df_inp_veh3, df_inp_cycle2, 0,
                                      MotorEffData1)
[df_op_33, df_op_scalars_33] = sim_EV(df_inp_veh3, df_inp_cycle3, 0,
                                      MotorEffData1)
[df_op_43, df_op_scalars_43] = sim_EV(df_inp_veh3, df_inp_cycle4, 0.2016,
                                      MotorEffData1)
[df_op_51, df_op_scalars_51] = sim_EV(df_inp_veh3, df_inp_cycle4, 0,
                                      MotorEffData1)
[df_op_63, df_op_scalars_63] = sim_EV(df_inp_veh3, df_inp_cycle5, 0,
                                      MotorEffData1)
[df_op_73, df_op_scalars_73] = sim_EV(df_inp_veh3, df_inp_cycle6, 0,
                                      MotorEffData1)
[df_op_83, df_op_scalars_83] = sim_EV(df_inp_veh3, df_inp_cycle7, 0.1228,
                                      MotorEffData1)
[df_op_93, df_op_scalars_93] = sim_EV(df_inp_veh3, df_inp_cycle8, 0,
                                      MotorEffData1)
